...
func (ur *UserRepository) Create(user models.User) error {
	return ur.repo.Execute(
		"INSERT INTO users (discord_id, username, joined_at) VALUES ($1, $2, $3) ON CONFLICT (discord_id) DO NOTHING", user.DiscordID, user.UserName, user.JoinedAt,
	)
}

func (ur *UserRepository) GetByID(id string) (models.User, error) {
	var user models.User
	row := ur.repo.QueryRow("SELECT discord_id, username FROM users WHERE discord_id = $1", id)
